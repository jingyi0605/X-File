import { useEffect, useLayoutEffect, useMemo, useRef, useState, type FormEvent, type MouseEvent as ReactMouseEvent, type PointerEvent as ReactPointerEvent, type ReactNode } from "react";
import type {
  HostDirectoryOption,
...
import { DesktopModal, ModalActions, ModalEmptyState, ModalField, ModalSection, ModalTag } from "../../shared/modal";
...
export function LibraryPage({ onOpenSettings, onOpenHealth, platformData }: LibraryPageProps) {
  ...
  async function pasteIntoTarget(target: LibraryContextMenuTarget): Promise<void> {
    ...
    const destinationPath = buildUniqueLibraryTargetPath(folderPath, getPathName(sourcePath), library.entries);
  ...
  async function runContextAction(actionId: LibraryContextActionId, target: LibraryContextMenuTarget): Promise<void> {
    const absolutePath = target.kind === "document" || target.kind === "folder" ? resolveTargetAbsolutePath(library, target) : null;
    switch (actionId) {
      case "preview":
  ...
  async function openDesktopLibraryContextMenu(event: ReactMouseEvent<HTMLElement>, target: LibraryContextMenuTarget): Promise<boolean> {
    if (platformData.runtimePlatform !== "desktop" || platformData.osFamily !== "macos") {
      return false;
    }

    const items = buildNativeLibraryContextMenuItems(library, target, libraryClipboard);
    if (items.length === 0) {
      return false;
    }
    ...
    try {
      ...
      const unlisten = await tauriEvent.listen<LibraryContextActionId>("x-file-library-context-menu-action", (actionEvent: NativeLibraryContextMenuActionEvent) => {
        handledByEvent = true;
        void runContextAction(actionEvent.payload, target).finally(() => {
          void unlisten();
        });
      });
      const result = await tauriApi.invoke<NativeLibraryContextMenuResult>("show_library_context_menu", {
        request: {
          items,
          x: event.clientX,
          y: event.clientY
  ...
  async function handleOpenLibraryContextMenu(event: ReactMouseEvent<HTMLElement>, target: LibraryContextMenuTarget): Promise<void> {
  ...
  if (shouldShowInitialization) {
    return <LibraryBindingPanel library={library} onOpenSettings={onOpenSettings} platformData={platformData} />;
  }
  ...
  return (
    <main className="app-shell workbench-shell xfile-workbench-shell" data-runtime-platform={platformData.runtimePlatform} data-os-family={platformData.osFamily} data-overlay-titlebar={platformData.overlayTitlebar ? "true" : undefined}>
      <section className="workbench-window xfile-workbench-window">
        ...
        <section className="affairs-main-panel">
          ...
          <LibraryStage
            ...
            onOpenContextMenu={(event, target) => void handleOpenLibraryContextMenu(event, target)}
            onRequestCreate={(state) => setPendingCreate(state)}
            onRequestRename={(path) => setPendingRename({ path, fileName: getPathName(path) })}
            onRequestDelete={(target) => setPendingDelete(target)}
            onRequestTagAssignment={(target) => setPendingTagAssignment(resolvePendingTagAssignmentTarget(target))}
          />
        </section>
        <LibraryDetail
          library={library}
      ...
      {pendingCreate ? <LibraryCreateModal library={library} state={pendingCreate} onChange={setPendingCreate} onClose={() => setPendingCreate(null)} /> : null}
  ...
  return (
    <aside className="workbench-sidebar affairs-layout-sidebar">
      ...
      <div className="workbench-sidebar-tools" aria-label={t("appTitle")}>
        <button type="button" className="workbench-tool-icon" aria-label={t("navLibrary")}>{renderSidebarGlyph("library")}</button>
        <button type="button" className="workbench-tool-icon" aria-label={t("navHealth")} onClick={onOpenHealth}>{renderSidebarGlyph("health")}</button>
        <button type="button" className="workbench-tool-icon" aria-label={t("navSettings")} onClick={onOpenSettings}>{renderSidebarGlyph("settings")}</button>
      </div>

      <div className="workbench-mode-switch" role="group" aria-label={t("xfileModeSwitchLabel")}>
        <button type="button" disabled>{t("xfileCodeMode")}</button>
        <button type="button" className="active">{t("xfileLibraryMode")}</button>
      </div>
      ...
      <div className="affairs-sidebar-panel">
        <div className="affairs-sidebar-shell">
          <div className="affairs-sidebar-content">
            <div className="affairs-sidebar-groups affairs-library-sidebar-groups">
              <div className="affairs-sidebar-group affairs-sidebar-group-plain affairs-sidebar-primary-nav">
                <SidebarPlainItem
                  active={library.viewState.browseMode === "folder" && !currentFolder}
              ...
              <div className="affairs-sidebar-group affairs-sidebar-group-plain affairs-favorites-panel">
                <div className="affairs-sidebar-group-header">
                  ...
                  <span className="affairs-sidebar-block-count">{snapshot?.favorites.length ?? 0}</span>
                </div>
                <div className="affairs-sidebar-list affairs-sidebar-list-plain">
                  {(snapshot?.favorites.length ?? 0) === 0 ? (
                    <p className="affairs-sidebar-empty">{t("libraryEmptyFavorite")}</p>
                  ) : (
              ...
              <div className="affairs-sidebar-group affairs-sidebar-group-plain affairs-tag-tree-panel">
                <div className="affairs-sidebar-group-header">
                  ...
                  <span className="affairs-sidebar-block-count">{snapshot?.folders.length ?? 0}</span>
                </div>
                <div className="affairs-sidebar-list affairs-sidebar-list-plain">
          ...
          <footer className="workbench-sidebar-footer">
            <button type="button" className="workbench-sidebar-footer-button" onClick={onOpenSettings}>{renderSidebarGlyph("settings")}{t("navSettings")}</button>
...
function SidebarPlainItem({
  ...
  return (
    <button type="button" className={active ? "affairs-sidebar-item active" : "affairs-sidebar-item"} onClick={onClick}>
      <span className="affairs-sidebar-item-button">
        <span className="affairs-sidebar-item-row">
          <span className="affairs-sidebar-leading-icon" aria-hidden="true">{icon === "folder" ? renderMiniFolderIcon() : renderTagIcon()}</span>
          <span className="affairs-sidebar-item-title" title={title}>{title}</span>
...
function LibraryBindingPanel({ library, onOpenSettings, platformData }: { library: LibraryState; onOpenSettings: () => void; platformData: WorkbenchPlatformData }) {
      ...
      <section className="library-init-page affairs-binding-shell">
        <div className="library-init-panel affairs-binding-card">
          ...
          <div className="library-init-body">
            <form className="library-init-form" onSubmit={(event) => void submit(event)}>
              <label>
                ...
                <div className="library-init-path-row">
                  ...
                  />
                  <button type="button" className="secondary-button" disabled={saving} onClick={openDirectoryBrowser}>
            ...
            <aside className="library-init-logo-panel" aria-label={t("appTitle")}>
              <img className="library-init-logo" src="/x-file-logo.svg" alt={t("appTitle")} />
            </aside>
          </div>
        </div>
      </section>
      <DesktopModal
        ...
        footer={
          <ModalActions align="between">
            <button type="button" className="secondary-button" disabled={browserLoading} onClick={closeDirectoryBrowser}>
              {t("actionCancel")}
            </button>
            <button type="button" className="primary-button" disabled={browserLoading || !browserCurrentPath} onClick={applyBrowserCurrentPath}>
      ...
      >
        <div className="host-directory-browser" aria-label={t("hostDirectoryBrowserTitle")}>
          ...
          <section className="host-directory-browser-panel">
            ...
            <div className="host-directory-browser-current" title={browserCurrentPath}>
              {browserCurrentPath || t("hostDirectoryNotLoaded")}
            </div>

            {browserError ? <div className="inline-alert compact">{browserError}</div> : null}
...
function LibraryStage({
  ...
  library: LibraryState;
  onOpenSettings: () => void;
  onOpenContextMenu: (event: ReactMouseEvent<HTMLElement>, target: LibraryContextMenuTarget) => void;
  ...
  const blankTarget: LibraryContextMenuTarget = { kind: "blank", folderPath: library.viewState.selectedFolderPath };
  ...
  return (
    <section className="affairs-stage-panel">
      ...
      />
      <div className="affairs-stage-content" onContextMenu={(event) => onOpenContextMenu(event, blankTarget)}>
        <div className="affairs-stage-meta-row">
          <span>{currentPath ? getPathName(currentPath) : t("libraryRootFolder")}</span>
          <span>{t("libraryCountDocuments", { count: library.documentPage?.total ?? library.snapshot?.documentCount ?? 0 })}</span>
          {directoryStatus ? <span>{resolveDirectoryStateLabel(directoryStatus.state)} · {resolveDirectorySourceLabel(directoryStatus.source)}</span> : null}
        </div>
        ...
        {library.hasMore ? (
          <div className="affairs-doc-grid-loading-overlay" aria-hidden="true">
            <button type="button" className="affairs-doc-grid-loading" disabled={library.documentsLoading} onClick={() => void library.loadMore()}>
              {t("libraryLoadMore")}
            </button>
          </div>
        ) : null}
...
function LibraryStageToolbar({
  ...
  library: LibraryState;
  directoryStatus: { state: LibraryDirectoryState; source: LibraryDirectorySource; errorSummary?: string | null } | null;
  onOpenSettings: () => void;
  onRequestCreate: (state: PendingCreateState) => void;
}) {
  return (
    <div className="affairs-stage-toolbar">
      <div className="affairs-stage-toolbar-left">
        <div className="affairs-stage-breadcrumb" aria-label={t("libraryCurrentFolder")}>
          ...
          <BreadcrumbItems path={library.viewState.browseMode === "folder" ? library.viewState.selectedFolderPath : library.viewState.selectedTagPath} browseMode={library.viewState.browseMode} onSelectFolder={library.selectFolder} onSelectTag={library.selectTag} />
        </div>
      </div>
      <div className="affairs-stage-toolbar-right">
        ...
        <div className="affairs-stage-toolbar-group">
          <button
            ...
            onClick={() => library.setViewState((current) => ({
              ...current,
              librarySort: { ...current.librarySort, direction: current.librarySort.direction === "desc" ? "asc" : "desc" as LibrarySortDirection }
            }))}
          >
            {library.viewState.librarySort.direction === "desc" ? t("librarySortDesc") : t("librarySortAsc")}
          </button>
        </div>
        <div className="affairs-stage-toolbar-group">
          <button type="button" className="affairs-stage-toolbar-icon" disabled={library.refreshPending} onClick={() => void library.refresh()} aria-label={t("libraryRefresh")} title={t("libraryRefresh")}>
            {renderRefreshIcon()}
          </button>
        </div>
        <div className="affairs-stage-toolbar-group">
          <button type="button" className="affairs-stage-status-trigger" title={resolveIndexStatusLabel(library.snapshot?.status.state)}>
            <span className={`affairs-stage-status-dot state-${resolveStatusDotState(library.snapshot?.status.state)}`} />
            {directoryStatus?.errorSummary ? <span className="affairs-stage-status-text">{directoryStatus.errorSummary}</span> : null}
          </button>
        </div>
        {library.viewState.browseMode === "folder" ? (
          <div className="affairs-stage-toolbar-group">
            <button type="button" className="affairs-stage-toolbar-segment" onClick={() => onRequestCreate({ kind: "directory", folderPath: library.viewState.selectedFolderPath, fileName: "" })}>{t("libraryCreateFolder")}</button>
            <button type="button" className="affairs-stage-toolbar-segment" onClick={() => onRequestCreate({ kind: "markdown", folderPath: library.viewState.selectedFolderPath, fileName: "" })}>{t("libraryCreateFile")}</button>
          </div>
        ) : null}
        <div className="affairs-stage-toolbar-group">
          <button type="button" className="affairs-stage-toolbar-icon" onClick={onOpenSettings} aria-label={t("navSettings")} title={t("navSettings")}>
            {renderSettingsIcon()}
          </button>
        </div>
      </div>
      ...
      {segments.map((segment, index) => {
        ...
        return (
          <span key={nextPath} className="affairs-stage-breadcrumb-fragment">
            ...
            <button
              type="button"
              className={current ? "affairs-stage-breadcrumb-button current" : "affairs-stage-breadcrumb-button"}
              onClick={() => browseMode === "folder" ? onSelectFolder(nextPath) : onSelectTag(nextPath)}
            >
              {segment}
            </button>
          </span>
  ...
  if (viewMode === "list") {
    return (
      <div className="affairs-stage-skeleton affairs-stage-skeleton-list" aria-hidden="true">
        {Array.from({ length: 12 }).map((_, index) => <div key={index} className="affairs-stage-skeleton-row" />)}
      </div>
    );
  }
  return (
    <div className="affairs-stage-skeleton affairs-stage-skeleton-grid" aria-hidden="true">
      {Array.from({ length: 24 }).map((_, index) => <div key={index} className="affairs-stage-skeleton-card" />)}
    </div>
  );
}
...
function VirtualLibraryGrid({
  ...
  const [measuredItemHeight, setMeasuredItemHeight] = useState(VIRTUAL_GRID_ITEM_HEIGHT);
  const [measuredRowGap, setMeasuredRowGap] = useState(VIRTUAL_GRID_ROW_GAP);

  useLayoutEffect(() => {
    const element = viewportRef.current;
    if (!element) return;
    const sync = () => setViewport((current) => ({ ...current, width: measureStageScrollContentWidth(element), height: element.clientHeight, scrollTop: element.scrollTop }));
    sync();
  ...
  useLayoutEffect(() => {
    ...
    const syncMeasurements = () => {
      ...
      if (next.itemHeight && Math.abs(next.itemHeight - measuredItemHeight) > 0.5) {
        setMeasuredItemHeight(next.itemHeight);
      }
      if (next.rowGap !== null && Math.abs(next.rowGap - measuredRowGap) > 0.5) {
  ...
  const columns = Math.max(
    1,
    measuredColumns ?? resolveAffairsGridColumnCount(viewport.width, {
      trackMinWidth: VIRTUAL_GRID_TRACK_MIN_WIDTH,
      columnGap: VIRTUAL_GRID_COLUMN_GAP
    })
  );
  const metrics = computeVirtualGridMetrics(entries.length, viewport.width, viewport.height, viewport.scrollTop, {
    columns,
  ...
  const shouldVirtualize = shouldVirtualizeAffairsGrid(entries.length, viewport.width, viewport.height, {
    itemHeight: measuredItemHeight,
    trackMinWidth: VIRTUAL_GRID_TRACK_MIN_WIDTH
  });
  const visibleSlots = shouldVirtualize
    ? buildVirtualLibraryEntrySlots(entries, metrics.startIndex, metrics.endIndex)
    : buildVirtualLibraryEntrySlots(entries, 0, entries.length);

  function handleScroll(element: HTMLDivElement): void {
    setViewport((current) => ({ ...current, scrollTop: element.scrollTop, width: measureStageScrollContentWidth(element), height: element.clientHeight }));
    if (!library.hasMore || library.documentsLoading) return;
    const remaining = element.scrollHeight - element.scrollTop - element.clientHeight;
    const preloadRows = Math.max(columns * 3, 18);
    if (metrics.endIndex >= entries.length - preloadRows || remaining <= VIRTUAL_LOAD_MORE_DISTANCE) {
  ...
  const grid = (
    <div
      className={shouldVirtualize ? "affairs-doc-grid affairs-doc-grid-virtual" : "affairs-doc-grid"}
      style={shouldVirtualize ? { top: `${metrics.offsetTop}px`, gridTemplateColumns: `repeat(${columns}, minmax(116px, 1fr))` } : undefined}
    >
      {visibleSlots.map((slot) => slot.entry ? (
        <LibraryEntryCard
  ...
  return (
    ...
    >
      <div className="affairs-doc-grid-viewport">
        {shouldVirtualize ? (
          <div className="affairs-doc-grid-spacer" style={{ height: `${metrics.totalHeight}px` }}>
            {grid}
          </div>
        ) : grid}
      </div>
...
function VirtualLibraryFinderList({
  ...
  const [measuredRowHeight, setMeasuredRowHeight] = useState(VIRTUAL_LIST_ROW_HEIGHT);
  ...
  useLayoutEffect(() => {
    ...
    const sync = () => setViewport((current) => ({
      ...
      scrollTop: clampScrollTop(element.scrollTop, entries.length, measuredRowHeight, element.clientHeight)
    }));
    sync();
    const observer = typeof ResizeObserver === "undefined" ? null : new ResizeObserver(sync);
    observer?.observe(element);
  ...
  const metrics = computeVirtualListMetrics(entries.length, viewport.height, viewport.scrollTop, { rowHeight: measuredRowHeight });
  const visibleSlots = buildVirtualLibraryEntrySlots(entries, metrics.startIndex, metrics.endIndex);

  function handleScroll(element: HTMLDivElement): void {
    const nextScrollTop = clampScrollTop(element.scrollTop, entries.length, measuredRowHeight, element.clientHeight);
    if (nextScrollTop !== element.scrollTop) {
      element.scrollTop = nextScrollTop;
    }
    setViewport((current) => ({ ...current, scrollTop: nextScrollTop, height: element.clientHeight }));
    if (!library.hasMore || library.documentsLoading) return;
    const remaining = element.scrollHeight - element.scrollTop - element.clientHeight;
    const preloadRows = Math.max(12, Math.ceil(Math.max(viewport.height, measuredRowHeight) / measuredRowHeight));
    if (metrics.endIndex >= entries.length - preloadRows || remaining <= VIRTUAL_LOAD_MORE_DISTANCE) {
      void library.loadMore();
    }
  }

  function handleFinderColumnResizeStart(column: FinderColumnKey, event: ReactPointerEvent<HTMLSpanElement>): void {
    event.preventDefault();
    event.stopPropagation();
    finderResizeStateRef.current = {
      column,
      startX: event.clientX,
      startWidth: library.viewState.finderColumnWidths[column] ?? DEFAULT_FINDER_COLUMN_WIDTHS[column]
    };
    ...
    const handlePointerMove = (moveEvent: PointerEvent) => {
      ...
      if (!current || current.column !== column || !Number.isFinite(moveEvent.clientX)) {
        return;
      }
  ...
  return (
    <div className="affairs-finder-shell">
      <div className="affairs-finder-header" style={{ gridTemplateColumns: finderGridTemplateColumns }}>
        {[
          ...
          <span key={column.key} className="affairs-finder-header-cell affairs-finder-cell">
            <button
              type="button"
              className="affairs-finder-sort-button"
              onClick={() => handleFinderSort(column.key as FinderColumnKey)}
              aria-label={buildFinderSortButtonLabel(column.label, library.viewState.librarySort, column.key as FinderColumnKey)}
              title={buildFinderSortButtonLabel(column.label, library.viewState.librarySort, column.key as FinderColumnKey)}
            >
              <span>{column.label}</span>
              {renderFinderSortIndicator(library.viewState.librarySort, column.key as FinderColumnKey)}
            </button>
            <span
              className="affairs-finder-column-resizer"
              role="separator"
              aria-orientation="vertical"
              aria-label={t("libraryColumnResizeLabel", { column: column.label })}
              onPointerDown={(event) => handleFinderColumnResizeStart(column.key as FinderColumnKey, event)}
            />
          </span>
        ))}
      </div>
      <div
        ...
        onScroll={(event) => handleScroll(event.currentTarget)}
      >
        <div className="affairs-finder-spacer" style={{ height: `${metrics.totalHeight}px` }}>
          <div className="affairs-finder-virtual" style={{ top: `${metrics.offsetTop}px` }}>
            {visibleSlots.map((slot) => slot.entry ? (
              <LibraryFinderRow
                key={`${slot.entry.kind}:${slot.entry.kind === "folder" ? slot.entry.path : slot.entry.documentId}`}
                entry={slot.entry}
              ...
              <AffairsFinderPlaceholderRow key={`list-placeholder-${slot.index}`} gridTemplateColumns={finderGridTemplateColumns} />
            ))}
          </div>
        </div>
      </div>
...
function resolveAffairsGridColumnCount(
  ...
  const trackMinWidth = Math.max(1, options?.trackMinWidth ?? VIRTUAL_GRID_TRACK_MIN_WIDTH);
  const columnGap = Math.max(0, options?.columnGap ?? VIRTUAL_GRID_COLUMN_GAP);
  return Math.max(1, Math.floor((Math.max(0, containerWidth) + columnGap) / (trackMinWidth + columnGap)));
}
...
function shouldVirtualizeAffairsGrid(
  ...
  const itemHeight = Math.max(1, options?.itemHeight ?? VIRTUAL_GRID_ITEM_HEIGHT);
  const trackMinWidth = Math.max(1, options?.trackMinWidth ?? VIRTUAL_GRID_TRACK_MIN_WIDTH);
  return itemCount >= VIRTUAL_GRID_MIN_ITEMS && viewportWidth >= trackMinWidth && viewportHeight >= itemHeight * 2;
}
...
function computeVirtualGridMetrics(
  ...
  const columns = Math.max(1, options?.columns ?? resolveAffairsGridColumnCount(viewportWidth, options));
  const itemHeight = Math.max(1, options?.itemHeight ?? VIRTUAL_GRID_ITEM_HEIGHT);
  ...
  const endRow = Math.min(rowCount, startRow + visibleRows + VIRTUAL_OVERSCAN * 2);
  return {
    columns,
...
function computeVirtualListMetrics(
  ...
  const visibleRows = Math.max(1, Math.ceil(Math.max(viewportHeight, rowHeight) / rowHeight));
  const startRow = Math.max(0, Math.floor(scrollTop / rowHeight) - VIRTUAL_LIST_OVERSCAN);
  const endRow = Math.min(itemCount, startRow + visibleRows + VIRTUAL_LIST_OVERSCAN * 2);
  return {
    startIndex: startRow,
    endIndex: endRow,
    offsetTop: startRow * rowHeight,
...
function clampScrollTop(scrollTop: number, itemCount: number, rowHeight: number, viewportHeight: number): number {
  const maxScrollTop = Math.max(0, itemCount * Math.max(1, rowHeight) - Math.max(0, viewportHeight));
...
function readMeasuredTrackCount(gridTemplateColumns: string | null | undefined): number | null {
  ...
  const tracks = normalized.split(/\s+/).map((item) => item.trim()).filter(Boolean);
...
function measureAffairsFinderRowHeight(container: HTMLElement | null): number | null {
  ...
  return rectHeight > 0 ? rectHeight : readMeasuredPixelValue(window.getComputedStyle(row).height);
}
...
function renderFinderSortIndicator(sortState: LibrarySortState, column: FinderColumnKey) {
  if (sortState.mode !== mapFinderColumnToSortMode(column)) {
    return <span className="affairs-finder-sort-indicator" aria-hidden="true" />;
  }
  return (
...
function buildFinderSortButtonLabel(label: string, sortState: LibrarySortState, column: FinderColumnKey): string {
  if (sortState.mode !== mapFinderColumnToSortMode(column)) {
    return t("librarySortByColumn", { column: label });
  }
  return t(sortState.direction === "asc" ? "librarySortColumnAsc" : "librarySortColumnDesc", { column: label });
}

function buildFinderGridTemplateColumns(widths: Record<FinderColumnKey, number>): string {
  const normalized = {
...
function measureAffairsGridLayout(container: HTMLElement | null): { columns: number | null; itemHeight: number | null; rowGap: number | null } {
  if (!container) {
  ...
  return {
    ...
    itemHeight: itemHeight > 0 ? itemHeight : readMeasuredPixelValue(item ? window.getComputedStyle(item).height : null),
    rowGap: readMeasuredPixelValue(gridStyles?.rowGap)
  };
}

function buildVirtualLibraryEntrySlots(loadedEntries: LibraryEntry[], startIndex: number, endIndex: number): VirtualLibraryEntrySlot[] {
...
function AffairsGridPlaceholderCard() {
  return (
    <div className="affairs-doc-item grid is-placeholder" aria-hidden="true">
      <div className="affairs-doc-icon"><div className="affairs-doc-placeholder-icon" /></div>
      <div className="affairs-doc-placeholder-lines"><span /><span /></div>
      <div className="affairs-doc-footer"><span className="affairs-doc-placeholder-meta" /></div>
    </div>
  );
}
...
function AffairsFinderPlaceholderRow({ gridTemplateColumns }: { gridTemplateColumns: string }) {
  return (
    <div className="affairs-finder-row is-placeholder" style={{ gridTemplateColumns }} aria-hidden="true">
      <span className="affairs-finder-name-cell"><span className="affairs-finder-placeholder-icon" /><span className="affairs-finder-placeholder-name" /></span>
...
function LibraryEntryCard({
  ...
  if (entry.kind === "folder") {
    ...
    return (
      <button
        type="button"
        className={active ? "affairs-doc-item grid active" : "affairs-doc-item grid"}
        onClick={() => handleFolderClick(library, entry.path)}
        onDoubleClick={() => library.selectFolder(entry.path)}
        onContextMenu={(event) => { library.selectFolderEntry(entry.path); onOpenContextMenu(event, { kind: "folder", entry }); }}
      >
        <div className="affairs-doc-icon">{renderFolderShape()}</div>
        <div className="affairs-doc-title" title={entry.name}>{entry.name}</div>
        <div className="affairs-doc-footer">
          <span className="affairs-doc-muted">{t("libraryCountDocuments", { count: entry.documentCount })}</span>
        </div>
      </button>
    );
  }
  ...
  return (
    <button
      ...
      onContextMenu={(event) => { library.selectDocument(entry.documentId); onOpenContextMenu(event, { kind: "document", entry }); }}
      onDoubleClick={() => void library.openPreview(entry.path)}
    >
      <div className="affairs-doc-icon">{renderDocumentShape(entry.path)}</div>
      <div className="affairs-doc-title" title={entry.title || t("libraryUntitled")}>{entry.title || t("libraryUntitled")}</div>
      <div className="affairs-doc-footer">
        <span className="affairs-doc-muted">{formatDateTime(entry.updatedAt)}</span>
      </div>
    </button>
  );
}
...
function LibraryFinderRow({
  ...
  if (entry.kind === "folder") {
    ...
    return (
      <button
        type="button"
        className={active ? "affairs-finder-row affairs-finder-directory-row active" : "affairs-finder-row affairs-finder-directory-row"}
        style={{ gridTemplateColumns }}
        onClick={() => handleFolderClick(library, entry.path)}
        onDoubleClick={() => library.selectFolder(entry.path)}
        onContextMenu={(event) => { library.selectFolderEntry(entry.path); onOpenContextMenu(event, { kind: "folder", entry }); }}
      >
        <span className="affairs-finder-name-cell"><span className="affairs-finder-icon">{renderFolderShape("row")}</span><span className="affairs-finder-name" title={entry.name}>{entry.name}</span></span>
        <span className="affairs-finder-cell">--</span>
        <span className="affairs-finder-cell">{formatDateTime(entry.updatedAt)}</span>
        <span className="affairs-finder-cell">{t("libraryFinderKindFolder")}</span>
        <span className="affairs-finder-cell">--</span>
      </button>
    );
  }
  ...
  return (
    <button
      ...
      onDoubleClick={() => void library.openPreview(entry.path)}
    >
      <span className="affairs-finder-name-cell"><span className="affairs-finder-icon">{renderDocumentShape(entry.path, "row")}</span><span className="affairs-finder-name" title={entry.title || t("libraryUntitled")}>{entry.title || t("libraryUntitled")}</span></span>
      <span className="affairs-finder-cell">{formatBytes(entry.sizeBytes)}</span>
      <span className="affairs-finder-cell">{formatDateTime(entry.updatedAt)}</span>
      <span className="affairs-finder-cell">{resolveDocumentVisual(entry.path).extension.toUpperCase()}</span>
      <span className="affairs-finder-cell">{formatDateTime(entry.createdAt)}</span>
    </button>
  );
}
  ...
  return (
    <aside className="affairs-detail-panel library-detail">
      <header className="affairs-detail-tabs" role="tablist" aria-label={t("libraryDetails")}>
        <button type="button" className="active">{t("libraryObjectDetail")}</button>
        <button type="button" disabled>{t("libraryAssistant")}</button>
      </header>
      {!selected ? (
        ...
        <div className="affairs-detail-scroll">
          <section className="affairs-detail-block affairs-detail-summary-block">
            <span className="affairs-detail-eyebrow">{t("libraryDetails")}</span>
            <div className="affairs-detail-title-block">
              <div className="affairs-doc-icon detail-doc-icon">{renderDocumentShape(selected.path, "row")}</div>
              <h2>{selected.title || t("libraryUntitled")}</h2>
              {selected.summary ? <p>{selected.summary}</p> : null}
            </div>
            <button type="button" className="affairs-detail-link-button">{t("libraryExpandText")}</button>
          ...
          <section className="affairs-detail-block">
            <div className="affairs-detail-actions-grid">
              <button type="button" className="secondary-button" onClick={() => void library.openPreview(selected.path)}>{t("libraryPreview")}</button>
              <button type="button" className="secondary-button" onClick={() => void library.downloadSelected(selected.path)}>{t("libraryDownload")}</button>
              <button type="button" className="secondary-button" onClick={() => onRequestRename(selected.path)}>{t("libraryRename")}</button>
              <button type="button" className="danger-button" onClick={() => onRequestDelete({ kind: "document", entry: selected })}>{t("libraryDelete")}</button>
            </div>
          </section>
...
function LibraryContextMenu({
  ...
  const folderPath = resolvePasteDestinationFolder(target);
  const absolutePath = isFileSystemTarget ? resolveTargetAbsolutePath(library, target) : null;
  ...
  return (
    ...
    >
      {isDocument ? <button type="button" role="menuitem" onClick={() => void run(() => { library.selectDocument(target.entry.documentId); return library.openPreview(target.entry.path); })}>{t("libraryContextPreview")}</button> : null}
      {isFileSystemTarget ? <button type="button" role="menuitem" onClick={() => void run(() => openContextTarget(library, target))}>{t("libraryContextOpen")}</button> : null}
      {isFileSystemTarget ? <button type="button" role="menuitem" onClick={() => void run(() => target.kind === "document" ? locateDocumentFolder(library, target.entry.path) : library.selectFolder(target.entry.path))}>{t("libraryContextLocate")}</button> : null}
      {isDocument && absolutePath ? <button type="button" role="menuitem" onClick={() => void run(async () => { await openPathInDesktop(absolutePath); })}>{t("libraryContextOpenLocalApp")}</button> : null}
      {isDocument ? <button type="button" role="menuitem" onClick={() => void run(() => library.downloadSelected(target.entry.path))}>{t("libraryContextDownload")}</button> : null}
      {target.kind === "folder" ? <button type="button" role="menuitem" onClick={() => void run(() => toggleFolderFavorite(library, target.entry))}>{isFolderFavorite(library, target.entry.path) ? t("libraryFavoriteRemove") : t("libraryFavoriteAddFolder")}</button> : null}
      {isBlankTarget ? (
        <ContextSubmenu
          menuKey="new"
        ...
        >
          <button type="button" role="menuitem" onClick={() => void run(() => onRequestCreate({ kind: "directory", folderPath, fileName: resolveDefaultCreateName("directory") }))}>{t("libraryContextNewDirectory")}</button>
          <button type="button" role="menuitem" onClick={() => void run(() => onRequestCreate({ kind: "markdown", folderPath, fileName: resolveDefaultCreateName("markdown") }))}>{t("libraryContextNewMarkdown")}</button>
          <button type="button" role="menuitem" onClick={() => void run(() => onRequestCreate({ kind: "text", folderPath, fileName: resolveDefaultCreateName("text") }))}>{t("libraryContextNewText")}</button>
          <button type="button" role="menuitem" onClick={() => void run(() => onRequestCreate({ kind: "custom", folderPath, fileName: resolveDefaultCreateName("custom") }))}>{t("libraryContextNewCustom")}</button>
        </ContextSubmenu>
      ) : null}
      {isFileSystemTarget ? (
        <ContextSubmenu
          ...
          onCloseLater={closeSubmenuLater}
        >
          <button type="button" role="menuitem" onClick={() => void run(() => copyContextText(resolveContextPath(target)))}>{t("libraryContextCopyFile")}</button>
          <button type="button" role="menuitem" onClick={() => void run(() => copyContextText(getContextTargetTitle(target)))}>{t("libraryContextCopyFileName")}</button>
          <button type="button" role="menuitem" disabled={!absolutePath} onClick={() => void run(() => copyContextText(absolutePath ?? ""))}>{t("libraryContextCopyAbsolutePath")}</button>
          <button type="button" role="menuitem" onClick={() => void run(() => copyContextText(resolveContextPath(target)))}>{t("libraryContextCopyRelativePath")}</button>
        </ContextSubmenu>
      ) : null}
      {isFileSystemTarget ? <button type="button" role="menuitem" onClick={() => void run(() => onSetClipboard({ mode: "cut", target }))}>{t("libraryContextCut")}</button> : null}
      <button type="button" role="menuitem" disabled={!canPaste} onClick={() => void run(pasteIntoTarget)}>{t("libraryContextPaste")}</button>
      {isFileSystemTarget ? <button type="button" role="menuitem" className="danger" onClick={() => void run(() => onRequestDelete(target))}>{t("libraryContextDelete")}</button> : null}
      {isFileSystemTarget ? <button type="button" role="menuitem" onClick={() => void run(() => onRequestTagAssignment(target))}>{t("libraryContextTags")}</button> : null}
      {isBlankTarget ? <button type="button" role="menuitem" onClick={() => void run(() => library.reloadDocuments(true))}>{t("libraryContextRefresh")}</button> : null}
      <button type="button" role="menuitem" onClick={() => void run(() => selectContextProperties(library, target))}>{t("libraryContextProperties")}</button>
    </div>
  );
}
...
function ContextSubmenu({
  ...
  return (
    ...
    >
      <button type="button" role="menuitem" aria-haspopup="menu" aria-expanded={activeSubmenu === menuKey}>
        <span>{label}</span>
...
function LibraryCreateModal({
  ...
  async function submit(): Promise<void> {
    ...
    try {
      await library.operateFile({
        ...
        content: state.kind === "directory" ? null : resolveCreateInitialContent(state.kind)
      });
      onClose();
    } catch (err) {
      setError(toApiErrorMessage(err));
  ...
  return (
    <DesktopModal
      ...
      description={t("libraryCreateModalDescription", { path: state.folderPath || t("libraryRootFolder") })}
      onClose={onClose}
      footer={(
        <ModalActions>
          <button type="button" className="secondary-button" disabled={submitting} onClick={onClose}>{t("actionCancel")}</button>
          <button type="button" className="primary-button" disabled={submitting} onClick={() => void submit()}>{submitting ? t("settingsSaving") : t("libraryCreateConfirm")}</button>
        </ModalActions>
      )}
    >
      <ModalField label={t("libraryCreateNameLabel")} description={resolveCreateKindLabel(state.kind)}>
        <input value={state.fileName} autoFocus placeholder={resolveCreatePlaceholder(state.kind)} onChange={(event) => onChange({ ...state, fileName: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter") void submit(); }} />
      </ModalField>
...
function LibraryRenameModal({
  ...
  async function submit(): Promise<void> {
    ...
    setError(null);
    try {
      await library.operateFile({ opType: "move", srcPath: state.path, dstPath: joinPath(getParentPath(state.path), fileName) });
      onClose();
    } catch (err) {
      setError(toApiErrorMessage(err));
    } finally {
  ...
  return (
    <DesktopModal
      ...
      footer={(
        <ModalActions>
          <button type="button" className="secondary-button" disabled={submitting} onClick={onClose}>{t("actionCancel")}</button>
          <button type="button" className="primary-button" disabled={submitting} onClick={() => void submit()}>{submitting ? t("settingsSaving") : t("libraryRenameConfirm")}</button>
        </ModalActions>
      )}
    >
      <ModalField label={t("libraryCreateNameLabel")}>
        <input value={state.fileName} autoFocus onChange={(event) => onChange({ ...state, fileName: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter") void submit(); }} />
      </ModalField>
      {error ? <div className="affairs-binding-hint affairs-create-error">{error}</div> : null}
    </DesktopModal>
  );
...
function LibraryDeleteModal({ library, target, onClose }: { library: LibraryState; target: LibraryContextMenuTarget; onClose: () => void }) {
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <DesktopModal
      ...
      footer={(
        <ModalActions>
          <button type="button" className="secondary-button" disabled={submitting} onClick={onClose}>{t("actionCancel")}</button>
          <button type="button" className="danger-button" disabled={submitting} onClick={() => void submit()}>{submitting ? t("settingsSaving") : t("libraryDeleteConfirmAction")}</button>
        </ModalActions>
      )}
    >
...
function LibraryTagAssignmentModal({ library, target, onClose }: { library: LibraryState; target: PendingTagAssignmentTarget; onClose: () => void }) {
  const [value, setValue] = useState("");
  const [details, setDetails] = useState<LibraryDocumentTagDetails | LibraryFolderTagDetails | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    const request = target.kind === "document"
  ...
  async function submit(): Promise<void> {
    ...
    try {
      if (target.kind === "document") {
        await saveDocumentTags(target.documentId, { createTagPaths: splitTagInput(value) });
      } else {
        await saveFolderTags({ folderPath: target.folderPath, createTagPaths: splitTagInput(value) });
      }
  ...
  return (
    <DesktopModal
      ...
      description={t("libraryTagAssignmentModalDescription", { name: target.title })}
      onClose={onClose}
      dismissible={!saving}
      footer={(
        <ModalActions>
          <button type="button" className="secondary-button" disabled={saving} onClick={onClose}>{t("actionCancel")}</button>
          <button type="button" className="primary-button" disabled={saving || loading} onClick={() => void submit()}>{saving ? t("settingsSaving") : t("librarySaveTags")}</button>
        </ModalActions>
      )}
    >
      {loading ? <ModalEmptyState title={t("libraryTagAssignmentLoading")} compact /> : null}
      <ModalField label={t("libraryTagAssignmentTagsLabel")} description={t("libraryTagAssignmentTagsDescription")}>
        <input
          value={value}
          autoFocus
      ...
      {recommendations.length ? (
        <ModalSection heading={t("libraryTagAssignmentRecommendations")} className="affairs-tag-assignment-recommendations">
          <div className="tag-group">
            {recommendations.map((item) => (
              <button
                ...
                onClick={() => setValue((current) => appendTagPath(current, item.path))}
                title={item.evidence}
              >
...
function PreviewPanel({ preview, loading, error }: { preview: LibraryPreview | null; loading: boolean; error: string | null }) {
  ...
  if ((preview.kind === "pdf" || preview.kind === "html") && preview.previewUrl) {
    return <a className="preview-link" href={preview.previewUrl} target="_blank" rel="noreferrer">{t("libraryPreviewOpenResource")}</a>;
  }

  if (preview.kind === "office" && preview.onlyOffice) {
    return (
      <div className="preview-box">
        <a className="preview-link" href={preview.onlyOffice.documentUrl} target="_blank" rel="noreferrer">{t("libraryPreviewOnlyOffice")}</a>
        <small>{preview.onlyOffice.editorMode === "edit" ? t("libraryPreviewOfficeEdit") : t("libraryPreviewOfficeView")}</small>
      </div>
    );
  }

  return <pre className="preview-box text">{preview.content || t("libraryPreviewEmpty")}</pre>;
}
...
function FolderTagPanel({ library }: { library: LibraryState }) {
  ...
  return (
    <div className="folder-tag-panel">
      ...
      <button type="button" className="secondary-button" onClick={() => void save()}>{t("librarySaveTags")}</button>
      {message ? <small>{message}</small> : null}
    </div>
  );
}
...
function resolveIndexStatusLabel(state: LibraryIndexState | undefined): string {
  switch (state) {
    case "fresh": return t("libraryStatusFresh");
    case "stale": return t("libraryStatusStale");
    case "queued": return t("libraryStatusQueued");
    case "running": return t("libraryStatusRunning");
    case "queue_timeout": return t("libraryStatusQueueTimeout");
    case "cooldown": return t("libraryStatusCooldown");
    case "failed": return t("libraryStatusFailed");
    default: return t("libraryStatusUnknown");
  }
}
...
function resolveContextMenuPosition(
...
): { left: number; top: number; width: number; maxHeight: number; transformOrigin: string } {
  ...
  return {
    left: opensLeft ? Math.max(margin, point.x - width) : Math.min(point.x, viewport.width - width - margin),
    top: opensUp ? Math.max(margin, point.y - height) : Math.min(point.y, viewport.height - height - margin),
    width,
    maxHeight,
...
function handleFolderClick(library: LibraryState, path: string): void {
  const openBehavior = library.snapshot?.binding?.folderOpenBehavior === "single_click" ? "single_click" : "double_click";
...
function getContextTargetTitle(target: Extract<LibraryContextMenuTarget, { kind: "document" | "folder" }>): string {
  return target.kind === "document" ? target.entry.title || getPathName(target.entry.path) : target.entry.name || getPathName(target.entry.path);
}

function resolveTargetAbsolutePath(library: LibraryState, target: Extract<LibraryContextMenuTarget, { kind: "document" | "folder" }>): string | null {
  const root = library.snapshot?.binding?.rootDir.trim().replace(/\/+$/g, "") ?? "";
...
function buildUniqueLibraryTargetPath(destinationFolder: string | null, fileName: string, entries: LibraryEntry[]): string {
  ...
  for (let index = 2; index < 1000; index += 1) {
    const candidate = joinPath(destinationFolder, `${name} ${index}${extension}`);
...
async function locateDocumentFolder(library: LibraryState, path: string): Promise<void> {
...
function buildNativeLibraryContextMenuItems(
  ...
  if (isDocument) {
    items.push({ id: "open-local-app", label: t("libraryContextOpenLocalApp"), disabled: !absolutePath });
  ...
  if (isFileSystemTarget) {
    items.push({
      ...
      items: [
        ...
        { id: "copy-absolute-path", label: t("libraryContextCopyAbsolutePath"), disabled: !absolutePath },
        { id: "copy-relative-path", label: t("libraryContextCopyRelativePath") }
      ]
    });
    items.push({ id: "cut", label: t("libraryContextCut") });
  }

  items.push({ id: "paste", label: t("libraryContextPaste"), disabled: !libraryClipboard });
...
async function openContextTarget(library: LibraryState, target: Extract<LibraryContextMenuTarget, { kind: "document" | "folder" }>): Promise<void> {
...
function isFolderFavorite(library: LibraryState, path: string): boolean {
  return Boolean(library.snapshot?.favorites.some((item) => item.kind === "folder" && item.path === path));
}

async function toggleFolderFavorite(library: LibraryState, entry: LibraryFolderEntry): Promise<void> {
  await library.toggleFavorite({ kind: "folder", path: entry.path, label: entry.name || getPathName(entry.path) });
}

function selectContextProperties(library: LibraryState, target: LibraryContextMenuTarget): void {
  if (target.kind === "document") {
    library.selectDocument(target.entry.documentId);
    return;
  }
  library.selectFolderEntry(target.kind === "folder" ? target.entry.path : library.viewState.selectedFolderPath);
}

function resolvePendingTagAssignmentTarget(target: LibraryContextMenuTarget): PendingTagAssignmentTarget | null {
  if (target.kind === "document") {
    return {
      kind: "document",
...
function normalizeCreateFileName(value: string, kind: PendingCreateKind): string {
...
function resolveDefaultCreateName(kind: PendingCreateKind): string {
  switch (kind) {
    case "directory": return t("libraryCreateDirectoryDefaultName");
    case "markdown": return t("libraryCreateMarkdownDefaultName");
    case "text": return t("libraryCreateTextDefaultName");
    default: return t("libraryCreateCustomDefaultName");
  }
}

function resolveCreateKindLabel(kind: PendingCreateKind): string {
  switch (kind) {
    case "directory": return t("libraryCreateKindDirectory");
    case "markdown": return t("libraryCreateKindMarkdown");
    case "text": return t("libraryCreateKindText");
    default: return t("libraryCreateKindCustom");
  }
}

function resolveCreatePlaceholder(kind: PendingCreateKind): string {
  switch (kind) {
    case "directory": return t("libraryCreateFolderPlaceholder");
    case "markdown": return t("libraryCreateMarkdownPlaceholder");
    case "text": return t("libraryCreateTextPlaceholder");
    default: return t("libraryCreateCustomPlaceholder");
  }
}
...
function resolveTagAssignmentInitialValue(details: LibraryDocumentTagDetails | LibraryFolderTagDetails): string {
  if ("resolvedTags" in details) {
    return details.resolvedTags.map((item) => item.path).filter(Boolean).join(", ");
  }
  return details.bindings.map((item) => item.tagPath).filter(Boolean).join(", ");
}
...
function resolveStatusDotState(state: LibraryIndexState | undefined): string {
  ...
  if (state === "running" || state === "queued" || state === "stale") return "running";
...
function renderMiniFolderIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M1.8 4.8h4.3l1.1 1.3h8v7.1a1.4 1.4 0 0 1-1.4 1.4H2.2A1.4 1.4 0 0 1 .8 13.2V6.2c0-.8.2-1.4 1-1.4Z" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>;
}

function renderTagIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.2 7.2V3.5c0-.7.6-1.3 1.3-1.3h3.7c.4 0 .7.1 1 .4l5.4 5.4a1.4 1.4 0 0 1 0 2l-3.6 3.6a1.4 1.4 0 0 1-2 0L2.6 8.2c-.3-.3-.4-.6-.4-1Z" fill="none" stroke="currentColor" strokeWidth="1.4"/><circle cx="5.3" cy="5.3" r="1" fill="currentColor"/></svg>;
}

function renderSidebarGlyph(kind: "library" | "health" | "settings") {
  if (kind === "settings") return renderSettingsIcon();
  if (kind === "health") return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.5 8.5h2.2l1.2-3.1 2.2 6.2 1.7-4.1h3.7" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>;
  return renderMiniFolderIcon();
}

function renderHomeIcon() {
  return <svg className="affairs-stage-breadcrumb-home-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="M2.2 7.4 8 2.7l5.8 4.7" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/><path d="M4.1 6.9v6h3.1V9.5h1.6v3.4h3.1v-6" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}

function renderGridIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.5 2.5h4v4h-4zm7 0h4v4h-4zm-7 7h4v4h-4zm7 0h4v4h-4z" fill="currentColor"/></svg>;
}

function renderListIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M3 4h10M3 8h10M3 12h10" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>;
}

function renderRefreshIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M12.6 5.1A5 5 0 0 0 3.2 6M3.4 10.9a5 5 0 0 0 9.4-1.1" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M12.6 2.7v2.8H9.8M3.4 13.3v-2.8h2.8" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}

function renderSettingsIcon() {
  return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 10.4A2.4 2.4 0 1 0 8 5.6a2.4 2.4 0 0 0 0 4.8Z" fill="none" stroke="currentColor" strokeWidth="1.3"/><path d="M13.5 8a5.6 5.6 0 0 0-.1-1l1.2-1-.9-1.6-1.5.5a5 5 0 0 0-1.7-1L10.2 2H6.3l-.3 1.5a5 5 0 0 0-1.7 1l-1.5-.5-.9 1.6 1.2 1a5.6 5.6 0 0 0 0 2L1.9 10l.9 1.6 1.5-.5a5 5 0 0 0 1.7 1l.3 1.5h3.9l.3-1.5a5 5 0 0 0 1.7-1l1.5.5.9-1.6-1.2-1c.1-.3.1-.7.1-1Z" fill="none" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}

function renderSearchIcon() {
  return <svg className="xfile-toolbar-search-icon" viewBox="0 0 16 16" aria-hidden="true"><path d="m11.2 11.2 2.3 2.3" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><circle cx="7.2" cy="7.2" r="4.5" fill="none" stroke="currentColor" strokeWidth="1.4"/></svg>;
}

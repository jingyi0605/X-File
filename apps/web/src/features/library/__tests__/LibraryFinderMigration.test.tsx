...
describe("第 7 批：列表/Finder 视图、类型文案、列宽和虚拟滚动", () => {
  ...
  it("目录模式下列表总条数会把文件夹数量算进去，不会被文档 visibleEntryTotal 截短", async () => {
    mockFinderViewportMetrics();
    libraryApiMock.getLibrarySnapshot.mockResolvedValue(
      createLibrarySnapshot({
        folders: [
          { path: "资料夹A", name: "资料夹A", parentPath: null, directDocumentCount: 0, documentCount: 0 },
          { path: "资料夹B", name: "资料夹B", parentPath: null, directDocumentCount: 0, documentCount: 0 },
        ],
      }),
    );
    libraryApiMock.listLibraryFiles.mockResolvedValue(
      createFileList([
        createFileNode({ path: "资料夹A", name: "资料夹A", kind: "directory" }),
        createFileNode({ path: "资料夹B", name: "资料夹B", kind: "directory" }),
      ]),
    );
    libraryApiMock.listLibraryDocuments.mockResolvedValue(
      createDocumentList(
        [
          createDocumentRecord({ documentId: "doc-1", path: "根目录文档1.md" }),
          createDocumentRecord({ documentId: "doc-2", path: "根目录文档2.md" }),
        ],
        { total: 2, visibleEntryTotal: 2 },
      ),
    );
